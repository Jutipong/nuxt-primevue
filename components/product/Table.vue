...
function onSearch(val: ProductSearch) {
    if (!val.name) {
        stateMock.customers = stateMock2.customers
        return
    }

    stateMock.customers = stateMock.customers.filter(r => r.name.includes(val.name!))
}
